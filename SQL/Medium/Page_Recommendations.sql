...
(
                ...
                SELECT
                        user1_id AS combined_data
                FROM
                        Friendship
                WHERE user2_id = 1

)AS xx
JOIN Likes ON Likes.user_id = xx.combined_data
WHERE  
        CASE WHEN page_id IN ( SELECT page_id FROM Likes WHERE user_id = 1 ) 
                THEN FALSE
                ELSE TRUE
        END